...
public struct AddShapeCommand: Command, Sendable {
    ...
    public static func execute(instruction: Shape, on model: inout Document) throws -> (doneCommand: AddShapeCommand, hadEffect: Bool) {
...
public struct DeleteShapeCommand: Command, Sendable {
    ...
    public static func execute(instruction: ShapeID, on model: inout Document) throws -> (doneCommand: DeleteShapeCommand, hadEffect: Bool) {
        guard let idx = model.indexOfShape(id: instruction) else { return (DeleteShapeCommand(id: instruction, removed: .rect(.rect(UUID()), Rect(x: 0, y: 0, width: 0, height: 0, color: .black)), removedIndex: -1), false) }
...
public struct MoveShapeCommand: Command, Sendable {
    ...
    public struct Instruction: Sendable {
        ...
        public init(id: ShapeID, offset: Point) { self.id = id; self.offset = offset }
        public init(id: ShapeID, dx: Double, dy: Double) { self.id = id; self.offset = Point(x: dx, y: dy) }
    }

    let id: ShapeID
    let offset: Point

    public static func execute(instruction: Instruction, on model: inout Document) throws -> (doneCommand: MoveShapeCommand, hadEffect: Bool) {
        ...
        if shape == originalShape { return (MoveShapeCommand(id: instruction.id, offset: .zero), false) }
    ...
    public func undo(on model: inout Document) {
        _ = try? MoveShapeCommand.execute(instruction: .init(id: id, offset: Point(x: -offset.x, y: -offset.y)), on: &model)
    }
...
public struct SetColorCommand: Command, Sendable {
    ...
    public struct Instruction: Sendable {
        ...
        public init(id: ShapeID, color: Color) { self.id = id; self.color = color }
    }
    ...
    public static func execute(instruction: Instruction, on model: inout Document) throws -> (doneCommand: SetColorCommand, hadEffect: Bool) {
        guard let idx = model.indexOfShape(id: instruction.id) else { throw VectorError.notFound }
        switch model.shapes[idx] {
        case .rect(let id, var r):
            if r.color == instruction.color { return (SetColorCommand(id: instruction.id, oldColor: r.color, newColor: r.color), false) }
            let old = r.color
            r.color = instruction.color
            model.shapes[idx] = .rect(id, r)
            return (SetColorCommand(id: instruction.id, oldColor: old, newColor: instruction.color), true)
        case .circle(let id, var c):
            if c.color == instruction.color { return (SetColorCommand(id: instruction.id, oldColor: c.color, newColor: c.color), false) }
            let old = c.color
...
public struct SelectShapeCommand: Command, Sendable {
    ...
    public static func execute(instruction: ShapeID?, on model: inout Document) throws -> (doneCommand: SelectShapeCommand, hadEffect: Bool) {
...
public enum CompositePresets {
    ...
    public static func duplicateAndMove(id: ShapeID, offset: Point) -> CompositeCommand<Document>.Instruction {
        return { model, exec in
            ...
            let (addCmd, _) = try exec.execute(command: AddShapeCommand.self, instruction: duplicated, on: &model)
            _ = try exec.execute(command: MoveShapeCommand.self, instruction: .init(id: addCmd.shape.id, offset: offset), on: &model)
            _ = try exec.execute(command: SelectShapeCommand.self, instruction: addCmd.shape.id, on: &model)
        }
    }
    
    // Convenience wrapper for backward compatibility
    public static func duplicateAndMove(id: ShapeID, dx: Double, dy: Double) -> CompositeCommand<Document>.Instruction {
